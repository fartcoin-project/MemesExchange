    ...
    utils.GetSessionStatus(req, status => {
        ...
        utils.CheckCoin(unescape(req.body.coin), err => {
            ...
            g_constants.dbTables['balance'].selectAll('*', WHERE, '', (err, rows) => {
                ...
                if (fullAmount*1 < 0.00000001) return onError(req, res, 'Bad order total ( total < 0.00000001 ) '+'( '+fullAmount*1+' < 0.00000001 )');
...
exports.GetAllOrders = function(coinsOrigin, callback)
{
    ...
    if (allOrders[coin0]) delete allOrders[coin0];
        ...
        g_constants.dbTables['orders'].selectAll('SUM(amount*1) AS amount, coin, price, time', 'coin="'+escape(coin0)+'" AND buysell="buy" AND amount*price>0 AND amount*1>0 ', 'GROUP BY price*100000000 ORDER BY price*100000000 DESC LIMIT 30', (err, rows) => {
        g_constants.dbTables['orders'].selectAll('SUM(amount*1) AS amount, coin, price, time', 'coin="'+escape(coin0)+'" AND buysell="sell" AND amount*price>0 AND amount*1>0 ', 'GROUP BY price*100000000 ORDER BY price*100000000 LIMIT 30', (err2, rows2) => {
...
function ValidateOrderRequest(req)
{
    ...
    {
        req['message'] = 'Bad order amount ( amount < 0.00000001 ) '+'( '+req.body.amount*1+' < 0.00000001 )';
        return false;
    }
    if (req.body.price*1 < 0.00000001)
    {
        req['message'] = 'Bad order price ( price < 0.00000001 )'+' ( '+req.body.price*1+' < 0.00000001 )';
        return false;
    }
    return true;
}
...
exports.ProcessExchange = function(coin)
{
    ...
    const SQL = 'SELECT * FROM (' +
                'SELECT * FROM (SELECT ROWID as id, * FROM orders where coin="'+coin+'"  AND amount*price > 0 AND amount*100>0.000000001 AND price*100>0.000000001 AND buysell="buy" ORDER BY price*1 DESC, time*1 LIMIT 1 ) '+
                'UNION ' +
                'SELECT * FROM (SELECT ROWID as id, * FROM orders where coin="'+coin+'"  AND amount*price > 0 AND amount*100>0.000000001 AND price*100>0.000000001 AND buysell="sell"  ORDER BY price*1, time*1 LIMIT 1 )' +
                ') ORDER BY buysell';
