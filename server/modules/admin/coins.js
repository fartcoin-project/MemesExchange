...
/*exports.onDaemonStart = function(ws, req, data)
{
    utils.GetSessionStatus(req, status => {
        if (!status.active || status.id != 1 || !data || !data.path || data.path.indexOf('/') == -1)
            return ws.send(JSON.stringify({request: 'rpc_responce', message: {result: false, data: 'MemesExchange: Bad request'}}));
            
        SendRPC(data.coin, data.command, data.params, ret => {
            if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({request: 'rpc_responce', message: {data: 'MemesExchange: Bad request'}}));
        });
    });
